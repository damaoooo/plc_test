...
def probe_operator_file(file_name: str, converter: Converter, queue: Queue):
    G = nx.Graph(pgv.AGraph(file_name))
...
def filter_useful_function(dataset: dict, threshold=2):
    ...
    for binary in dataset['data']:
        bad_function_list = []
        for func in dataset['data'][binary]:
            if len(dataset['data'][binary][func]) < threshold:
                bad_function_list.append(func)

        for func in bad_function_list:
            del dataset['data'][binary][func]
            
    # Delete binary that contains no useful function
    bad_binary_list = []
    for binary in dataset['data']:
        useful = False
        for function in dataset['data'][binary]:
            if len(dataset['data'][binary][function]) >= threshold:
                useful = True
                break
            
        if not useful:
            bad_function_list.append(binary)
...
def collector(queue: Queue, control: Queue, all_data: dict):
    ...
    while 1:
        ...
        if not control.empty():
            ...
            if x == "finish":
                ...
                if not x:
                    continue
                # print("Got", x['name'], "from:", x['binary'], len(all_data['data']))
    ...
    all_data['adj_len'] = adj_len
    all_data['feature_len'] = feature_len

    print("Finish")
    
def probe_convert_handler(queue: Queue, op_file: str, converter: Converter):
    missing_op = set()
    while 1:
        if not queue.empty():
            new_op = queue.get()
            if new_op == "finish":
                ...
                missing_op.add(new_op)
                print("Detected new OP:", new_op)
            
    converter.OP = list(missing_op)
    converter.save_op_list(op_file)
    print("Saved new OP:", missing_op, "to", op_file)
    
def probe_op_list(pwd: str, op_list: str, converter: Converter,queue: Queue, pool: Pool):
    threading.Thread(target=probe_convert_handler, args=(queue, op_list, converter), name="ProbeConvertHandler").start()
    for arch in os.listdir(pwd):
        if arch.startswith('cpg'):
            continue
        
        for binary_file in os.listdir(os.path.join(pwd, arch, 'lifted')):
            if not binary_file.startswith("c_dot_"):
                continue
            
            
            cpg_dir = os.path.join(pwd, arch, 'lifted', binary_file)

            for dot_file in os.listdir(cpg_dir):
                file_path = os.path.join(cpg_dir, dot_file)
                pool.apply(func=probe_operator_file, args=(file_path, converter, queue))
...
def run_it(pwd, save_dir, converter: Converter, queue: Queue, control: Queue, pool: Pool, read_cache: bool = True):
    ...
    for arch in os.listdir(pwd):
        ...
        for binary_file in os.listdir(os.path.join(pwd, arch, 'lifted')):
            ...
            binary_name = binary_file.replace("c_dot_", "")
    ...
    random.seed(114514)
    train_set, test_set = split_dataset_by_binary(all_data, ratio=0.1)
...
if __name__ == '__main__':
    ...
    args.add_argument("--input", '-i', type=str,
                      help="the input dot directory", default="/ibex/tmp/zhoul0e/all")
    args.add_argument("--output", '-o', type=str,
                      help="The outout directory", default="/ibex/tmp/zhoul0e/all/cpg_file")
    ...
    # probe_op_list(pwd, op_list, Converter(), queue, pool)   
