...
class ASTGraphDataset(Dataset):
    ...
    def _prepare(self):
        ...
        for index in range(len(self.data)):
            ...
            result.append(
                [sample, same_sample, different_sample, torch.tensor([0])])
        return result
...
class ASTGraphDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self):
        adj_len, feature_len, train_total_length, offset, train_file_list = self._load_dataset_info(
            os.path.join(self.data_path, "train_set"))

        # Assume train_set and test_set are the same
        _, _, test_total_length, _, test_file_list = self._load_dataset_info(
            os.path.join(self.data_path, "test_set"))

        # total_dataset = ASTGraphDataset(data, max_adj=min(adj_len, 1000), feature_len=feature_len, exclude=self.exclude)

        self.train_set = ASTGraphDataset(data_path=train_file_list, offset=offset,
                                         length=train_total_length, max_adj=adj_len, feature_len=feature_len)
        self.val_set = ASTGraphDataset(data_path=test_file_list, offset=offset,
                                       length=test_total_length, max_adj=adj_len, feature_len=feature_len)
